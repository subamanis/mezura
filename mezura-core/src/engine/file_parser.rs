...
// 512, and past that the hits are the generators themselves, whose source holds the marker it
// prints. Reading further finds fewer generated files and more of those.
...
-> Result<FileOutcome,String>
{
    ...
    // Before the parse, which is what they save: a bundle is the most expensive file there is.
    // A file somebody named is counted whatever it holds, the way the walk counts a named one that
    // a '.gitignore' covers.
    if !written_by_hand {
...
mod tests {
    ...
    fn parse_file_report(path: &Path, lang_name: &str, buf: &mut String, config: &EngineConfig) -> Result<FileReport, String> {
        match parse_file(path, lang_name, buf, &mut ParseBuffers::default(), &shipped_lookup(),
                &mut KeywordMatchers::default(), config, false)? {
            FileOutcome::Counted(report) => Ok(report),
...
<style>\n/* css comment */\n</style>\n<p>bye</p>\n";
    ...
    // The test is the average line length and not where the first break falls: a bundle written
    // under a licence comment has an ordinary first line and a payload under it.
    #[test]
    fn a_bundle_is_left_out_of_the_counts_and_counted_when_the_flag_asks() {
        ...
        let skipped = |path: &std::path::Path, config: &EngineConfig| {
            ...
            matches!(parse_file(path, "JavaScript", &mut buf, &mut ParseBuffers::default(),
                    &shipped_lookup(), &mut KeywordMatchers::default(), config, false),
        ...
        // Long lines and nothing a report could feel, so it is never even measured
