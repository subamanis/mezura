...
// The definition files this crate ships, and the rule that settles a contested extension. Bytes and
// not parsed languages, because the installer writes them to disk unchanged and parses one at a time
// to ask whether a user's copy still means what ours means. Plain tuples and not the embedder's own
// file type, so that a release of 'include_dir' is never a breaking change of ours.
pub fn shipped_language_files() -> Vec<(&'static str, &'static [u8])> {
...
pub fn shipped_extension_priority() -> &'static [u8] {
