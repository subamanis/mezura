...
// own numbers still hold it and the counts do not. The count of them is the only thing that says
// so, which is why it is on the result and not merely printed.
#[test]
fn a_minified_file_is_of_interest_counted_by_nothing_and_reported() {
    let root = std::env::temp_dir().join("mezura-minified-run-test");
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).unwrap();
    std::fs::write(root.join("bundle.js"), format!("var a{};\n", "x".repeat(4000)).repeat(20)).unwrap();
    std::fs::write(root.join("app.js"), "var a = 1;\nvar b = 2;\n").unwrap();
    ...
    // and the flag counts it as it is written, which is one line of code per line of payload
...
// The rule the walk already keeps for an ignored, dotted or linked path: what somebody wrote out
// is counted whatever it holds. What a pattern matched was found by the program, so it is not.
#[test]
fn a_file_named_as_a_target_is_counted_however_it_looks() {
    let root = std::env::temp_dir().join("mezura-named-target-test");
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).unwrap();
    let bundle = root.join("bundle.js");
    let generated = root.join("api.pb.go");
    std::fs::write(&bundle, format!("var a{};\n", "x".repeat(4000)).repeat(20)).unwrap();
    std::fs::write(&generated, "// Code generated by protoc-gen-go. DO NOT EDIT.\n\npackage api\n").unwrap();
    let named = |path: &std::path::Path| path.to_str().unwrap().replace('\\', "/");
    ...
    // and the same two files reached through a pattern are found by the program, so they are not
    // named and the skipping applies
    let config = EngineConfig { threads: Threads::new(1, 2),
    ...
    assert!(result.nothing_of_interest_was_counted());
    assert!(!result.all_relevant_files_were_faulty(), "nothing failed to parse");
}

#[test]
fn a_generated_file_is_of_interest_counted_by_nothing_and_reported() {
    let root = std::env::temp_dir().join("mezura-generated-run-test");
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).unwrap();
    std::fs::write(root.join("api.pb.go"), "// Code generated by protoc-gen-go. DO NOT EDIT.\n\npackage api\n").unwrap();
