...
- Minified files are left out of the counts by default, and the report says how many were. One bundle counts as a single line of code while the source it was built from is tens of thousands, so a project with one in it gets a figure that is wrong in both directions at once: its JavaScript looks tiny and its average line enormous. The test is the average bytes per line, 1000 or more, and files under 10 KB are never tested since they cannot move a report whatever they hold. The figure sits far above real source on purpose: the widest genuine files measured, generated bindings whose declarations are padded into columns, reach 350, while the bundles that actually hide work behind two lines are in the thousands. ```--count-minified``` counts every file as it is written.

- Generated files are left out too, and reported the same way. Their lines are perfectly ordinary, so no measurement of shape can find them; what finds them is that the tools which write them say so, and the test is exactly that: ```do not edit```, ```auto-generated```, ```autogenerated``` or ```@generated``` anywhere in the first 512 bytes. That catches protobuf output, hardware register maps, ORM models, bindings and precomputed tables. The window is small on purpose rather than to save time: read deeper and what turns up is the generators themselves, whose own source holds the marker they print into their output. ```--count-generated``` counts them. What no marker can catch is a generated file whose author left no note, and there is no honest way to find those.

- Batch writes its comments as ```rem```, and mezura looks for that word anywhere on a line without asking whether it is a whole word. A line whose first word merely begins with those three letters, ```remote.exe /q```, is read as a comment. A line with a real word before them, ```del C:\remote\file.txt```, is code as it should be, since the line has already said something. Over 183 batch files the case did not occur once, and it is written down here because it can.
