...
pub fn counting_settings(config: &mezura::engine::config::EngineConfig) -> [(&'static str, String); 8] {
    let mezura::engine::config::EngineConfig { dirs, exclude_dirs, languages_of_interest, excluded_languages,
...
pub fn log_stats(path: &str, contents: &Option<String>, result: &mezura::RunResult, datetime_now: &DateTime<Local>, config: &Configuration) -> io::Result<()> {
...
fn write_current_log(writer: &mut BufWriter<File>, config: &Configuration, datetime_now: &DateTime<Local>, result: &mezura::RunResult) -> io::Result<()> {
    ...
    if result.has_modules() {
        ...
        for module in &result.modules {
            ...
            writer.write_all(format!("        {}:\n", module.name.as_deref().unwrap_or(mezura::UNNAMED_MODULE_NAME)).as_bytes())?;
